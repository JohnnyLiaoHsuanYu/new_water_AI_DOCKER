...
log_path = os.path.join("/app", "log", "dr_model.log")
...
class ModelDataLoader:
    ...
    def load_region_data(self):
        re_path = os.path.join(self.base_path, 'county_lonlat_drop_repeat_points_with_resevior.csv')
    ...
    def load_resevior_data(self):
        resevior_rd_ = os.path.join(self.base_path, 'predict_resevior_percentage_withname.csv')
    ...
    def load_resevior_model(self):
        for renn in self.resevior_nums.keys():
            model_path = os.path.join(self.base_path, f'resevior_XGBRegressor_{renn}.joblib')
...
resevior_config = ToolKit.read_yaml('/app/resevoir_config.yaml')
...
work_path___ = '/app/'#'D:/2310011_Liao/水資源/docker_waterAI/' 
model_name_dr = 'waterAI_XGBRegressor_new.joblib'
data_loader = ModelDataLoader(work_path___, model_name_dr)
predic_name = model_name_dr[:-7]

final_file_path = lambda predic_path, time_stamp: os.path.join(predic_path, f'waterAI_result_{time_stamp}__.json')

file_name = final_file_path(predic_path, formatted_today)
url_rain = f'https://www.cwa.gov.tw/V8/C/C/Statistics/MonthlyData/MOD/{last_YY}_{last_mm}.html?'
logger.make_log(f'All the is prepared! the final file name is {file_name}')
...
if response.status_code == 200:
    ...
    logger.make_log(f"Error: Unable to fetch the PDF. Status code: {response.status_code}", 'ERROR')
...
if match_date:
    ...
    d_444 = datetime.datetime(match_year[-1], int(match_date[1][0]), int(match_date[1][1]))
...
ToolKit.write_file(file_name, js.dumps(output, ensure_ascii=False))
